...
## 2025-05-24 - Reverse Tabnabbing Prevention
**Vulnerability:** `<a>` tags with `target="_blank"` missing the `noopener` value in the `rel` attribute are vulnerable to Reverse Tabnabbing. This allows the newly opened tab to potentially exploit the `window.opener` API to redirect or manipulate the originating page.
**Learning:** Modern browsers mitigate this by default for `target="_blank"`, but explicitly defining `rel="noopener noreferrer"` remains a fundamental security best practice and is required to pass static security analysis tools and defend against older clients.
**Prevention:** Always verify that `<a>` tags using `target="_blank"` include `rel="noopener noreferrer"`.
